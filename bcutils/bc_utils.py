...
with open(os.path.join(SDIR, "config.json"), "r") as f:
...
def create_bc_session(config_obj: dict, do_login=True):
    ...
    if do_login:
        ...
        logging.info(
            f"GET {BARCHART_URL + 'login'}, status: {resp.status_code}, CSRF token: {csrf_token}"
        )
    ...
    try:
        ...
        if os.path.isfile(full_path):
            if file_is_placeholder_for_no_hourly_data(full_path):
                logging.info(
                    "Placeholder found indicating missing hourly data, switching to daily"
                )
                period = "daily"
            else:
                logging.info(
                    f"Data for contract '{contract}' already downloaded, skipping\n"
                )
        ...
        if tick_date is not None and start_date < tick_date:
            logging.info(
                f"Switching to daily prices for '{contract}', "
            ...
            if not dry_run:
                ...
                if resp.status_code == 200:
                    if "Error retrieving data" not in resp.text:
                        ...
                        logging.info(
                            f"Barchart data problem for '{instrument}_{datecode}00', not writing"
                        )
...
def get_barchart_downloads(
    ...
    try:
        ...
        if low_data_contracts:
            logging.warning(
                f"Low/poor data found for: {low_data_contracts}, maybe check config"
            )
...
def file_is_placeholder_for_no_hourly_data(path):
    ...
    if size < 150:
        ...
        if (
            len(df) == 2
            and check_row_date(df.index[-1])
            and check_row_date(df.index[-2])
        ):
            return True
...
if __name__ == "__main__":
    ...
    config = {
        "barchart_username": "freddiehill000@gmail.com",
        "barchart_password": "QVZJQqu8",
    }
    # bc_config = {k: os.environ.get(v) for k, v in config.items() if v in os.environ}
    # print(bc_config)

    get_barchart_downloads(
        create_bc_session(config_obj=config),
        contract_map=CONTRACT_MAP,
        save_directory="/Users/f1/Desktop/ib_data_download/barchart/daily",
        start_year=1975,
        end_year=2026,
        dry_run=True,
        force_daily=True,
